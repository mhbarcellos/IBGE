...
import { getEnvNumber, pciSourceName } from './pciUtils.mjs';

const require = createRequire(import.meta.url);
const { PDFParse } = require('pdf-parse');

console.log('Extraindo texto dos PDFs PCI...');
const supabase = await createSupabaseSeedClient();
const maxPdfs = getEnvNumber('PCI_MAX_PDFS', 6);
const textDir = resolve(process.cwd(), 'data/imported/pci/texts');
...
for (const file of files ?? []) {
  ...
  const localTextPath = `data/imported/pci/texts/${file.id}.txt`;
